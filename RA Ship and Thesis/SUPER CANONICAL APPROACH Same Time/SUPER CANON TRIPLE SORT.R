...
one=list(); two=list(); three=list(); four=list(); five=list(); six=list(); seven=list()
eight=list(); nine=list(); ten=list()
Choice_V=list(one, two, three, four, five, six, seven, eight, nine, ten)
rm(one, two, three, four, five, six, seven, eight, nine, ten)

for (i in 1:10) {
  for (j in 1:93) { #used to be 27
    Choice_V[[i]][[j]]=VRY[, colnames(VRY) %in% Choice[[i]][[j]]][j,]
  }
}
...
one=list(); two=list(); three=list(); four=list(); five=list(); six=list(); seven=list()
eight=list(); nine=list(); ten=list()
Choice_VOL_SIGNAL=list(one, two, three, four, five, six, seven, eight, nine, ten)
rm(one, two, three, four, five, six, seven, eight, nine, ten)

for (i in 1:10) {
  for (j in 1:93) {#used to be 27
    Choice_VOL_SIGNAL[[i]][[j]]=t(apply(Choice_V[[i]][[j]], 1, rank))
    Choice_VOL_SIGNAL[[i]][[j]]=t(ntile(Choice_VOL_SIGNAL[[i]][[j]], 3))
    colnames(Choice_VOL_SIGNAL[[i]][[j]])=colnames(Choice_VOL_SIGNAL[[i]][[j]])
  }
}
...
CHOICE_FINAL=list(First_Tercile,Second_Tercile,Third_Tercile)
rm(one, two, three, four, five, six, seven, eight, nine, ten,First_Tercile,
   Second_Tercile,Third_Tercile)

for (i in 1:3) {
  for (j in 1:10) { #THIS IS ONLY 1/1, 2/2, 3/3
    for (k in 1:93) {
      CHOICE_FINAL[[i]][[j]][[k]]=ifelse(Choice_N_SIGNAL[[j]][[k]]==i & 
                                           Choice_VOL_SIGNAL[[j]][[k]]==i
                                         , 1, 0)
    }
  }
}
...
for (i in 1:3) {
  for (j in 1:10) {
    for (k in 1:93) {#used to be 27
      Monthly_Second[[i]][[j]][[k]]=Monthly[, colnames(Monthly) %in%
                                              colnames(CHOICE_FINAL[[i]][[j]][[k]])]
    }
  }
}
...
for (i in 1:3) {
  for (j in 1:10) {
    for (z in 1:90) {#used to be 81, and not z+1
      
      Port[[i]][[j]][[z]]=rbind(Monthly_Second[[i]][[j]][[z]][z+1,]*xts(CHOICE_FINAL[[i]][[j]][[z]], 
                                                                        order.by = 
                                                                          index(Monthly_Second[[i]][[j]][[z]])[z+1]),
                                
                                Monthly_Second[[i]][[j]][[z]][z+2,]*xts(CHOICE_FINAL[[i]][[j]][[z]], 
                                                                        order.by = 
                                                                          index(Monthly_Second[[i]][[j]][[z]])[z+2]),
                                
                                Monthly_Second[[i]][[j]][[z]][z+3,]*xts(CHOICE_FINAL[[i]][[j]][[z]], 
                                                                        order.by = 
