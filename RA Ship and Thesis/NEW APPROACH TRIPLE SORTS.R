...
for (i in 1:10) {
  for (j in 1:27) {
    Choice_V[[i]][[j]]=VOL[, colnames(VOL) %in% Choice[[i]][[j]]][Seq[j],]
  }
}
...
for (i in 1:10) {
  for (j in 1:27) {
    ...
    colnames(Choice_VOL_SIGNAL[[i]][[j]])=colnames(Choice_N[[i]][[j]])
  }
}
...
for (i in 1:3) {
  for (j in 1:10) {
    for (z in 1:80) {#used to be 81, and not z+1
      ...
      Port[[i]][[j]][z]=apply(CHOICE_FINAL[[i]][[j]][[Seq[z]]]*
                                Monthly_Second[[i]][[j]][[Seq[z]]][z+1,], 1, sum)/
        sum(CHOICE_FINAL[[i]][[j]][[Seq[z]]])
    }
  }
}

test=1+unlist(Port[[3]][[1]])
prod(test)
...
for (i in 1:10) {
  for (j in 1:27) {
    Choice_V[[i]][[j]]=VRY[, colnames(VRY) %in% Choice[[i]][[j]]][Seq[j],]
  }
}
